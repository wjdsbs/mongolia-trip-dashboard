...
## 저용량 지형 지도와 환율 계산기 (2026-09)

지도 배경을 실제 도로·지형이 있는 Esri World Topo Map 이미지로 교체했습니다. `public/maps/`의 WebP 5장은 총 약 154KB이며, 화면에 선택한 지도 한 장(약 18–49KB)만 읽습니다. 하루 동안 브라우저 캐시를 사용하며 지도 SDK나 실시간 타일 요청은 없습니다. 전체 동선, 미니사막·숙소, 테를지, 시내 확대 버튼 및 번호순 장소 목록을 제공합니다. 이전 확대 박스 방식은 현재 화면에서 사용하지 않습니다.

- `scripts/build-basemaps.ts`: 배경 이미지 재생성(개발 시에만 외부 지도 요청).
- `src/generated/basemaps.json`: 배경의 Web Mercator 범위와 파일 크기.
- `src/generated/map-routes.json`: 위·경도 도로 경로. 기존 `maps:build`에서 함께 생성.
- 미니사막 엘승 타사르하이는 첫날 공항 → 미니사막 → Bichigt Khad camp 순서로 추가했습니다. 대표 좌표의 출처는 https://mapcarta.com/Elsen_Tasarkhai 이며 정확한 체험 집결지가 아닌 지역 대표 위치입니다.
- 신규 미니사막 구간은 도로 데이터가 없어 점선으로 표시합니다. 기존 로컬 도로 데이터는 유지합니다.
- 환율 계산기는 직접 입력한 `1 KRW = n MNT` 환율을 기기에 저장하고 양방향으로 계산합니다. 자동 환율 조회는 하지 않으며 수수료는 제외하고 정수로 반올림해 표시합니다.

브라우저가 연결되지 않은 환경에서는 실제 모바일 터치 화면 검증을 별도로 진행해야 합니다.
